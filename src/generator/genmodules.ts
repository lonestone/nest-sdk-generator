...
export function generateSdkModules(modules: SdkModules): RecordDict<string> {
  ...
  for (const [moduleName, controllers] of modules) {
    for (const [controllerName, controller] of controllers) {
      ...
      out.push('')
      out.push(`export const ${controller.registrationName} = {`)
    ...
    moduleContent.push("/// Auto-generated file (Boilerplate's SDK generator)")
    ...
    for (const controller of controllers.values()) {
      moduleContent.push(`import { ${controller.registrationName} } from "./${controller.registrationName}";`)
    }
